...
const projects = [
  /* ---------- software ---------- */
  {
    ...
    status: "wip",
    caption:
      "A jet-combat arena for Unity 6, cel-shaded. Four aircraft fly with real transonic behaviour and moving control surfaces. Every mesh, texture, sound and particle is generated by code at runtime, so there is nothing to download and no imported asset anywhere in it.",
  ...
  },
  {
    ...
    featured: true,
    caption:
      "Every reported drone and missile strike in the Russia-Ukraine war, day by day, with the outlet behind each figure. It reads a few dozen sources every morning and runs on its own server.",
  ...
  },
  {
    ...
    status: "live",
    caption:
      "A Chrome extension that grades every move of a chess game as it happens, on lichess and chess.com. The bands are chess.com's own, so Brilliant and Blunder mean what players already expect them to mean. Stockfish runs inside the extension and thinks for a fixed second per move rather than to a fixed depth. Openings are judged against a book of 3,328 named lines, so a gambit reads as theory and the badge lands before the engine is asked at all. It pauses in live games against people.",
  ...
  },
  {
    ...
    status: "live",
    caption:
      "Hold a coastal fortress against a campaign of named capital ships. You lay the guns yourself and mark what to hit, and the enemy keeps sailing. Released on itch.io as Penumbra.",
    tags: ["Python", "pygame"],
    links: [
  ...
  },
  {
    ...
    status: "wip",
    caption:
      "A deterministic flight-dynamics model in C++20: atmosphere, aircraft, autopilot, and radar, IRST and RWR sensors. The core is headless and has no rendering dependency, so a run replays identically on every machine. The game sits on top of it.",
    tags: ["C++", "CMake"],
  ...
  },
  {
    ...
    status: "wip",
    caption:
      "Watches a chessboard on your screen, rebuilds the position, and says what to play. The engine is being ported to C++ for speed, move for move against the Python one.",
  ...
  },
  {
    ...
    status: "live",
    caption:
      "The Russian card game, with transfers. Play the machine on this page, or open a table and play a friend browser to browser. There is no server and no account.",
  ...
  },
  {
    ...
    status: "live",
    caption:
      "Turns a fugue into a band. A fugue is already written as independent melodic lines, so instead of hacking chords apart it works out which line is which and hands each one to a player. One voice per instrument, the pedal line on the bass, strictly one note at a time. 3,445 movements of Bach are published as Guitar Pro files, and the 73 organ movements as 331 printable parts besides. Four Vivaldi arias sit at the end, and after them a fugue the program wrote itself.",
  ...
  },
  {
    ...
    status: "wip",
    caption:
      "Give it a PDF of a score and it hands back classical-guitar tablature under the notation, with an editor for the bars it misreads. It also transcribes from a recording, or from a video of a page.",
  ...
  },
  {
    ...
    status: "live",
    caption:
      "A Windows app that downloads YouTube videos and playlists as mp3 or mp4, merges a playlist into one file, and plays the songs into a virtual microphone for a voice call. It is one PowerShell file and a WPF window, with nothing to install beyond yt-dlp.",
  ...
  },
  {
    ...
    cat: "software",
    caption:
      "A-Level coursework: both hex board games in full, with sound and a computer opponent. Yavalath was itself designed by a program, which is what made it worth building.",
    tags: ["Python"],
    links: [
  ...
  },
  {
    ...
    cat: "software",
    caption:
      "Swiss editorial on black, static HTML, no build step. Every project gets its own page, and the previews are real screenshots. The commit heatmap and the feed under it pull from GitHub in your browser.",
  ...
  },
  {
    title: "Project Euler",
    cat: "software",
    caption: "Sixty-nine solved, each solution as it was written. The Python runs in your browser against the real input.",
    tags: ["Python", "Pyodide"],
    links: [
  ...
  },
  {
    ...
    cat: "software",
    status: "old",
    caption: "The first thing made in pygame. Click the circles before they shrink away.",
    tags: ["Python", "pygame"],
    links: [
  ...
  /* ---------- hardware ---------- */
  {
    ...
    cat: "hardware",
    caption:
      "A can-sized satellite for the CanSat competition, built by a team of seven. The antenna work above started here.",
  ...
  },
  {
    ...
    status: "wip",
    caption:
      "A 14.5 dBi Yagi-Uda feeding RF transceivers off a Raspberry Pi 3 to range a target. The antenna works – the ranging does not yet.",
  ...
  /* ---------- research ---------- */
  {
    ...
    cat: "research",
    caption:
      "Multilayer perceptrons written from scratch in NumPy, trained at a range of sizes to measure how test loss falls with parameter count. It reached 98.52% on MNIST and 93.35% on FashionMNIST. The library is on PyPI as elkwork and the paper runs to 56 pages.",
